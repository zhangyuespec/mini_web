...
def add_focus(ret):
    ...
    #判断是否有这个股票代码
    conn=connect(host="local",port=3306,user="root",password="123456",database="stock_db",charset="utf8")
    cs=conn.cursor()
    sql="select * from info where code=%s;"#为了防止ｓｑｌ注入，我们不能在这边写ｓｔｏｃｋ_code ，在下面写
    cs.execute(sql,(stock_code,))
    if not cs.fetchone():#如果没有这个股票代码，认为是非法请求
        cs.close()
        conn.close()
        return "没有这支股票代码"
    #如果查询到了
